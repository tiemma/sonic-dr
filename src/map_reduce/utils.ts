...
export const ensureDependenciesSatisfied = async (metadata: DBMetadataGraph, table: string) => {
    ...
    if (dependencySize > 0) {
        ...
        getLogger(`Dependencies fulfilled for table ${table}, expected=${dependencySize}, found=${filledTables["count"]}`)
    }
}
...
export const shutdown = async (processOrder: Queue) => {
    ...
    getLogger(getLoggerID())(`Processed ${processOrder.getElements().length} tables in the following order: ${processOrder.getElements()}`)
